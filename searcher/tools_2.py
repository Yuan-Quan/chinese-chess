...
import elephantfish_2 as elephantfish_2
...
def search(searcher, pos, secs, history=()):
    ...
    # print('score', score, 'depth', depth, 'secs', secs)
...
def can_kill_king(pos):
    ...
    return any(pos.value(m) >= elephantfish_2.MATE_LOWER for m in pos.gen_moves())


def mrender(pos, m):
    # Sunfish always assumes promotion to queen
    p = 'q' if elephantfish_2.A9 <= m[1] <= elephantfish_2.I9 and pos.board[m[0]] == 'P' else ''
    m = m if get_color(pos) == WHITE else (254 - m[0], 254 - m[1])
    return elephantfish_2.render(m[0]) + elephantfish_2.render(m[1]) + p


def mparse(color, move):
    m = (elephantfish_2.parse(move[0:2]), elephantfish_2.parse(move[2:4]))
...
def get_color(pos):
    ''' A slightly hacky way to to get the color from a elephantfish_2 position '''
...
def parseFEN(fen):
    ...
    score = sum(elephantfish_2.pst[p][i] for i, p in enumerate(board) if p.isupper())  # 计算大写字符即红方的子力价值
    score -= sum(elephantfish_2.pst[p.upper()][254 - i] for i, p in enumerate(board) if p.islower())  # 计算小写字符即黑方的子力价值
    pos = elephantfish_2.Position(board, score)
